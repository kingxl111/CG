...
struct Material {
    glm::vec3 ambient;  // Окружающее освещение
    glm::vec3 diffuse;  // Диффузное освещение
    glm::vec3 specular; // Зеркальное освещение
    float shininess;    // Гладкость для зеркального отражения
